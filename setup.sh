...
mkdir -p /var/lib/xox > /dev/null 2>&1
# // Ram Information
...
function pasang_domain() {
        ...
        if [[ $host == "1" ]]; then
                ...
                echo "IP=" >> /var/lib/xox/ipvps.conf
...
function make_folder_xray() {
        ...
        mkdir -p /etc/xox/limit/vmess/ip
        mkdir -p /etc/xox/limit/vless/ip
        mkdir -p /etc/xox/limit/trojan/ip
        mkdir -p /etc/xox/limit/ssh/ip
...
EOF
        ...
        #SERVICE VMESS
        # // Installing UDP Mini
        mkdir -p /usr/local/xox/
        wget -q -O /usr/local/xox/udp-mini "${REPO}limit/udp-mini"
        chmod +x /usr/local/xox/udp-mini
...
function ins_Fail2ban() {
        ...
        # Ganti Banner
        wget -O /etc/xox.txt "${REPO}limit/issue.net"
