...
parser.add_argument(
    "--protocols", "-l", action="store_true", help="Show list of protocols"
)
...
quick.add_argument(
    "--shadowsocks", "-ss", action="store_true", help="Create ShadowSocks"
)
...
def create_key():
    ...
    print(green)
    subprocess.run(
        "openssl req -new -newkey rsa:4096 -days 735 -nodes -x509     -subj"
        " '/C={}/ST=Denial/L=String/O=Dis/CN=www.{}.{}' -keyout {} -out {}".format(
...
def install_bbr() -> None:
    subprocess.run(
        "curl https://raw.githubusercontent.com/SonyaCore/across/master/bbr.sh |"
        " bash -",
        shell=True,
        check=True,
    )
...
def block_torrent_manually():
    iptables_cmds = [
        ...
        (
            'iptables -A INPUT -m string --string "announce.php?passkey=" --algo bm -j'
            " DROP"
        ),
        'iptables -A INPUT -m string --string "torrent" --algo bm -j DROP',
...
def loglevel():
    ...
    loglevel_messages = [
        ...
        (
            "usually some external problem that does not affect V2Ray but possibly the"
            " user experience."
        ),
...
def client_side_configuration(protocol):
    ...
    print(
        blue
        + "! Or use below one-line compact json Client-Side and import it directly in"
        " your client:",
        reset,
    )
    print(
...
def xray_create(protocol):
    ...
    if args.tls or args.vless or args.trojan:
        print(
            yellow
            + "! Using self-signed key        \n! Make sure to add Allow Insecure to"
            " your client."
            + reset
        )
...
def run_docker():
    ...
    try:
        ...
        if os.path.exists("/usr/bin/docker-compose") or os.path.exists(
            ...
            subprocess.run(
                "curl -SL"
                " https://github.com/docker/compose/releases/download/{}/docker-compose-linux-x86_64"
